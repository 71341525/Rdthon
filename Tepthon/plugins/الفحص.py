...
async def amireallyalive(event):
    ...
    start = datetime.now()
    zedevent = await edit_or_reply(event, "**✦ جـاري فحـص تنصيبـك لـ 𝗥𝗱𝘁𝗵𝗼𝗻 ⌬ . .**")
    end = datetime.now()
    ...
    ALIVE_TEXT = gvarstatus("ALIVE_TEXT") or "** بـوت ريدثون 𝗥𝗱𝘁𝗵𝗼𝗻 يعمـل بنجـاح ☑️ .. **"
...
**{Z_EMOJI} قنـاة الـسـورس :** [اضغـط هنـا](https://t.me/A0R01)"""
...
async def amireallyialive(event):
    ...
    reply_to_id = await reply_id(event)
    Z_EMOJI = gvarstatus("ALIVE_EMOJI") or "⎆╿"
    zed_caption = "** بـوت ريدثون 𝗥𝗱𝘁𝗵𝗼𝗻 يعمـل بنجـاح ☑️ .. **\n"
    zed_caption += f"**{Z_EMOJI} إصــدار تليثـون : ** `{version.__version__}\n`"
    zed_caption += f"**{Z_EMOJI} إصــدار ريدثون : ** `{tepversion}`\n"
